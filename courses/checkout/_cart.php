...
//global $wpdb;
//$array = $wpdb->get_results("select meta_value from " . $wpdb->prefix . "usermeta where meta_key='_woocommerce_persistent_cart_1'");
////print_r($array);
...
if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key)) {
    ...
    include(EDUQ_COURSE_PLUGIN_CHECKOUT_BASE . '_cart_item.php');
    if (!empty($cart_item["optional_courses"])) {
        foreach ($cart_item["optional_courses"] as $service) {
            include(EDUQ_COURSE_PLUGIN_CHECKOUT_BASE . '_cart_service.php');
        }
    }
}
?>