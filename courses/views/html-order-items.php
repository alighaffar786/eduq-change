    ...
    <table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
            ...
            foreach ($line_items as $item_id => $item) {
                ...
                if (!empty($item_optional_courses)) {
                    foreach ($item_optional_courses as $optional_item_id) {
                        $courseDate = "--";
                        $css_class_service  = "padding-left: 50px;";
                        $datax = "service-item";
        ...
        <tbody id="order_fee_line_items">
            ...
            foreach ($line_items_fee as $item_id => $item) {
                include EDUQ_WC_PLUGIN_ADMIN_META_BOX_BASE . '/html-order-fee.php';
            }
        ...
        <tbody id="order_shipping_line_items">
            ...
            foreach ($line_items_shipping as $item_id => $item) {
                include EDUQ_WC_PLUGIN_ADMIN_META_BOX_BASE . '/html-order-shipping.php';
            }
        ...
        <tbody id="order_refunds">
            ...
            if ($refunds) {
                foreach ($refunds as $refund) {
                    include EDUQ_WC_PLUGIN_ADMIN_META_BOX_BASE . '/html-order-refund.php';
                }
    ...
    <?php if ($order->get_total_refunded()) : ?>
        <table class="wc-order-totals" style="border-top: 1px solid #999; margin-top:12px; padding-top:12px">
            <tr>
                ...
                <td width="1%"></td>
                <td class="total refunded-total">-<?php echo wc_price($order->get_total_refunded(), array('currency' => $order->get_currency())); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                                    ?></td>
            </tr>
    ...
    <div class="wc-order-data-row wc-order-refund-items wc-order-data-row-toggle" style="display: none;">
        <table class="wc-order-totals">
            ...
            <tr>
                ...
                <td class="total">-<?php echo wc_price($order->get_total_refunded(), array('currency' => $order->get_currency())); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                    ?></td>
            </tr>
            <tr>
                <td class="label"><?php esc_html_e('Total available to refund', 'woocommerce'); ?>:</td>
                <td class="total"><?php echo wc_price($order->get_total() - $order->get_total_refunded(), array('currency' => $order->get_currency())); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                    ?></td>
            </tr>
            <tr>
                <td class="label">
...
<div class="woocommerce_order_items_wrapper">
    <h2 class="woocommerce-order-data__heading">
        <?php echo COURSE_CHECKOUT_LBL['no_of_courses'] . " (" . $total_no_students . ") "; ?>
    </h2>

    <table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
        <thead>
            <tr>
                <th colspan="7" data-sort="string-ins"><?php esc_html_e('Item', 'woocommerce'); ?></th>
            </tr>
            ...
            foreach ($line_items as $item_id => $item) {
                if (isset($item["info"])) {
                    ...
                    if (!isset($optional_courses[$product->get_ID()])) {
                        ...
                        echo "<td colspan='7'>" . $item->get_name() . "</td>";
                        ...
                        for ($i = 1; $i <= $info["noCursists"]; $i++) {
                            echo "<tr>";
                            echo "<td>" . $info["firstname_cursist" . $i] . "</td>";
                            echo "<td>" . $info["lastname_cursist" . $i] . "</td>";
                            echo "<td>" . $info["birtdate_cursist" . $i] . "</td>";
                            echo "</tr>";
                        }
                    }
                }
...
<style>
    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items tbody th,
    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items td.name.service-item {
        padding-left: 40px;
    }

    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items tbody th,
    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items td.name.service-item:before {
        content: "—";
    }

    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items tbody th,
    #woocommerce-order-items .woocommerce_order_items_wrapper table.woocommerce_order_items td.thumb.service-item div {
        display: none;
    }
</style>